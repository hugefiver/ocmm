...
Patterns evaluated in order. First match wins, except `hyperplan-ultrawork` which combines both.

| Type | Regex | Mode prompt source |
|---|---|---|
| `hyperplan-ultrawork` | `\b(hpp\|hyperplan)\s+(ulw\|ultrawork)\b` (and reverse) | `mode/hyperplan.md` + ultrawork variant (see below) |
| `ultrawork` | `\b(ultrawork\|ulw)\b` | ultrawork variant |
| `hyperplan` | `\b(hpp\|hyperplan)\b` | `mode/hyperplan.md` |
...
4. **`isPlannerAgent`** — for standalone `ultrawork` (not `hyperplan-ultrawork`), skip when the active agent is `prometheus` or `plan` (planners get planner.md instead).

## Ultrawork variant routing

When intent is `ultrawork` or `hyperplan-ultrawork`, choose the ultrawork prompt variant by this priority:

1. `isPlannerAgent(agentName)` → `prompts/ultrawork/planner.md`
2. `isGptModel(modelId)` → `prompts/ultrawork/gpt.md`
3. `isGeminiModel(modelId)` → `prompts/ultrawork/gemini.md`
4. _(else default — Claude/Kimi/GLM/etc.)_ → `prompts/ultrawork/default.md`
...
```
<mode prompt (hyperplan/team)>
<empty line>
<ultrawork variant prompt (gpt/gemini/default/planner)>
<empty line>
<original agent system prompt>
```

For `hyperplan-ultrawork`, both blocks are present. For other types, only the relevant one.
...
- `.kb/prompts/ultrawork/default.md` — full ultrawork (CODE RED, certainty protocol, TDD, …)
- `.kb/prompts/ultrawork/gpt.md` — decision-framework variant
- `.kb/prompts/ultrawork/gemini.md` — intent gate + tool mandate variant
- `.kb/prompts/ultrawork/planner.md` — concise plan-only
- `.kb/prompts/ultrawork/codex.md` — codex variant (used by codex edition; OCMM ignores Phase 1)
- `.kb/prompts/mode/hyperplan.md` — adversarial multi-agent planning
