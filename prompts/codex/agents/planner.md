...
When gathering context for a plan, batch independent calls only to permitted read-only utility profiles. Dispatch sequentially when one lookup's result is another's input. Never dispatch an implementation worker, Reviewer/Oracle profile, or decision agent from the planner role.
