...
| Architecture uncertainty | Gather evidence and compare concrete options directly; use `dw-hard-reasoning` only if genuinely difficult. Strict or high-risk conditions alone do not qualify. |
...
| Genuinely difficult decision after evidence gathering; strict or high-risk conditions alone do not qualify | Use `dw-hard-reasoning` with evidence and options. Runtime debugging stays in the debugging workflow. |
...
| dw-hard-reasoning agent | Genuinely difficult decision; strict or high-risk conditions alone do not qualify | Recommendation with tradeoffs |
