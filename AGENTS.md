...
The package also carries the Codex adapter marketplace at `.agents/plugins/marketplace.json` and the generated plugin bundle at `plugins/deepwork/`. When testing release/package install paths, verify both `codex plugin marketplace add <package-root>` and `codex plugin add deepwork@deepwork-local`; the Codex `.mcp.json` must keep the default `lsp` MCP plugin-local as `./dist/cli/ocmm-lsp.js` rather than baking local source, Cargo, `target/`, or marketplace-root-relative `../../dist` paths. The Codex bundle should expose the workflow skill as `deepwork` and generated agent profiles with the `dw-*` prefix, including `dw-oracle`, `dw-oracle-2nd`, and `dw-creative`; configured review `variants` may additionally emit logical tier profiles such as `dw-oracle-high`. `dw-reviewer` is primary-model or primary-lane implementation self-review; `dw-oracle` is the slot-1 logical-normal external-model cross-check, and `dw-oracle-2nd` is the default second-priority external Oracle slot when configured/available. Explicit user model configuration may remove model heterogeneity.

Planning profiles follow the same explicit-only bundle rule: unsuffixed `dw-planner` and `dw-plan-critic` are logical normal, while `dw-planner-low/high/max` and `dw-plan-critic-low/high/max` are generated only for explicitly configured canonical `planner` or `plan-critic` `variants`. Generated files are not callable-availability proof; release checks must inspect the current callable schema, verify absent suffix TOMLs stay absent by default, and verify generated inventory matches the configured profiles exactly. Every planning tier inherits its canonical role prompt, mode, permissions, and receipt behavior. The `plan-critic-low` profile may route to a lower-cost model, but its xhigh-equivalent floor is non-negotiable in both generated TOML and runtime output.
