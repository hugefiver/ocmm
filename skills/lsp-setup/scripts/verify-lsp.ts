...
// The engine source is located by walking up from this script and the cwd.
// It requires an upstream omo checkout that contains packages/lsp-tools-mcp/src.
...
class EngineNotFoundError extends Error {
	constructor(public readonly target: string) {
		super(`lsp-tools-mcp engine not found (looked for ${target}). Run verify-lsp.ts inside an upstream omo checkout, or use the current OpenCode lsp MCP diagnostics tool directly.`)
