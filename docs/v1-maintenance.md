...
| agents/planner.md | writing-plans | upstream Prometheus planner scope | `.omo`-specific plan-only command flow | local docs/superpowers plan path, writing-plans skill contract, brainstorm precondition in First Action, injected skill utilization section, tool-selection guidance (ast-grep/rg/fd/lsp_*/shell-aware terminal) |
